...
namespace Libmpc
{
    ...
    public class Mpc
    {
        ...
        /// </summary>
        /// <param name="filename">The name and path of the file to add.</param>
        /// <returns>The id of the file in the playlist.</rereturns>
        public int AddId(string filename)
        {
            if (filename == null)
                throw new ArgumentNullException("filename");

            MpdResponse response = this.getConnection().Exec("addid", new string[] { filename });
		...
		/// </summary>
		/// <returns>The meta data of the items in the current playlist.</returns>
		public List<MpdFile> PlaylistInfo(int start, int end) {
			MpdResponse response = this.getConnection().Exec("playlistinfo", new string[] { start.ToString() + ":" + end.ToString() });
